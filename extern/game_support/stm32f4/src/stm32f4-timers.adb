...
package body STM32F4.Timers is
   ...
   is
      Temp_CR1 : TIMx_CR1 := This.CR1;
   begin
      This.ARR := Period;
      This.Prescaler := Prescaler;
      Temp_CR1.Clock_Division := Clock_Divisor;
      Temp_CR1.Mode_And_Dir := Counter_Mode;
      This.CR1 := Temp_CR1;
   end Configure;
   ...
   function Current_Clock_Division (This : Timer) return Timer_Clock_Divisor is
      Temp_CR1 : constant TIMx_CR1 := This.CR1;
   begin
      return Temp_CR1.Clock_Division;
   ...
   begin
      ...
      --  TODO: verify need. see TIM_TimeBaseInit() in stm32f4xx_tim.c
   ...
   procedure Enable (This : in out Timer) is
      Temp : TIMx_CR1 := This.CR1;
   begin
      Temp.Timer_Enabled := True;
      This.CR1 := Temp;
   end Enable;
   ...
   function Enabled (This : Timer) return Boolean is
      Temp : constant TIMx_CR1 := This.CR1;
   begin
      return Temp.Timer_Enabled;
   end Enabled;
   ...
   function No_Outputs_Enabled (This : Timer) return Boolean is
      Temp : constant TIMx_CCER := This.CCER;
   begin
      for C in Channel_1 .. Channel_3 loop
         if Temp (C).CCxE = Enable or Temp (C).CCxNE = Enable then
            return False;
         end if;
      end loop;
      ...
      if Temp (Channel_4).CCxE = Enable then
         return False;
      end if;
      return True;
   ...
   procedure Enable_Main_Output (This : in out Timer) is
      Temp : TIMx_BDTR := This.BDTR;
   begin
      Temp.Main_Output_Enabled := True;
      This.BDTR := Temp;
   ...
   begin
      if No_Outputs_Enabled (This) then
         Temp.Main_Output_Enabled := False;
         This.BDTR := Temp;
      end if;
   ...
   function Main_Output_Enabled (This : Timer) return Boolean is
      Temp : constant TIMx_BDTR := This.BDTR;
   begin
      return Temp.Main_Output_Enabled;
   ...
   procedure Set_Autoreload (This : in out Timer;  Value : Half_Word) is
   begin
      This.ARR := Value;
   end Set_Autoreload;
   ...
   function Current_Autoreload (This : Timer) return Half_Word is
   begin
      return This.ARR;
   ...
   is
      Temp_DCR : TIMx_DCR := This.DCR;
   begin
      Temp_DCR.Base_Address := Base_Address;
      Temp_DCR.Burst_Length := Burst_Length;
      This.DCR := Temp_DCR;
   end Configure_DMA;
   ...
   is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Capture_Compare_DMA_Selection := True;
      This.CR2 := Temp_CR2;
   ...
   is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Capture_Compare_DMA_Selection := False;
      This.CR2 := Temp_CR2;
   ...
   is
      Temp : TIMx_CR1 := This.CR1;
   begin
      Temp.Update_Request_Source := Source /= Global;
      This.CR1 := Temp;
   ...
   is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Master_Mode_Selection := Source;
      This.CR2 := Temp_CR2;
   ...
   is
      Temp_SMCR : TIMx_SMCR := This.SMCR;
   begin
      Temp_SMCR.Slave_Mode_Selection := Mode;
      This.SMCR := Temp_SMCR;
   ...
   procedure Enable_Master_Slave_Mode (This : in out Timer) is
      Temp_SMCR : TIMx_SMCR := This.SMCR;
   begin
      Temp_SMCR.Master_Slave_Mode := True;
      This.SMCR := Temp_SMCR;
   ...
   procedure Disable_Master_Slave_Mode (This : in out Timer) is
      Temp_SMCR : TIMx_SMCR := This.SMCR;
   begin
      Temp_SMCR.Master_Slave_Mode := False;
      This.SMCR := Temp_SMCR;
   ...
   is
      Temp_SMCR : TIMx_SMCR := This.SMCR;
   begin
      Temp_SMCR.External_Trigger_Polarity := Polarity;
      Temp_SMCR.External_Trigger_Prescaler := Prescaler;
      Temp_SMCR.External_Trigger_Filter := Filter;
      This.SMCR := Temp_SMCR;
   ...
   begin
      Configure_External_Trigger (This, Polarity, Prescaler, Filter);
      Temp_SMCR.External_Clock_Enable := True;
      This.SMCR := Temp_SMCR;
   ...
   is
      Temp_SMCR : TIMx_SMCR := This.SMCR;
   begin
      Temp_SMCR.Trigger_Selection := Source;
      This.SMCR := Temp_SMCR;
   ...
   is
      Temp_CCER : TIMx_CCER := This.CCER;
   begin
      -- first disable the channel
      Temp_CCER (Channel).CCxE := Disable;
      This.CCER := Temp_CCER;

      Set_Output_Compare_Mode (This, Channel, Mode);

      Temp_CCER (Channel).CCxE := State;
      Temp_CCER (Channel).CCxP := Polarity'Enum_Rep;
   ...
   begin
      ...
      Temp_CCER (Channel).CCxE := State;
      Temp_CCER (Channel).CCxNP := Complementary_Polarity'Enum_Rep;
      Temp_CCER (Channel).CCxP := Polarity'Enum_Rep;

      case Channel is
         when Channel_1 =>
            Temp_CR2.Channel_1_Output_Idle_State := Idle_State;
            Temp_CR2.Channel_1_Complementary_Output_Idle_State := Complementary_Idle_State;
         when Channel_2 =>
            Temp_CR2.Channel_2_Output_Idle_State := Idle_State;
            Temp_CR2.Channel_2_Complementary_Output_Idle_State := Complementary_Idle_State;
         when Channel_3 =>
            Temp_CR2.Channel_3_Output_Idle_State := Idle_State;
            Temp_CR2.Channel_3_Complementary_Output_Idle_State := Complementary_Idle_State;
         when Channel_4 =>
            Temp_CR2.Channel_4_Output_Idle_State := Idle_State;
      end case;
   ...
   is
      Temp_CCER : TIMx_CCER := This.CCER;
   begin
      Temp_CCER (Channel).CCxNP := Polarity'Enum_Rep;
      This.CCER := Temp_CCER;
   ...
   function Channel_Enabled
     (This    : Timer;
      ...
      return Boolean
   is
      Temp_CCER : constant TIMx_CCER := This.CCER;
   begin
      return Temp_CCER (Channel).CCxE = Enable;
   end Channel_Enabled;
   ...
   is
      Temp_CCER : TIMx_CCER := This.CCER;
   begin
      Temp_CCER (Channel).CCxNE := Enable;
      This.CCER := Temp_CCER;
   ...
   is
      Temp_CCER : TIMx_CCER := This.CCER;
   begin
      Temp_CCER (Channel).CCxNE := Disable;
      This.CCER := Temp_CCER;
   ...
   is
      Temp_CCER : constant TIMx_CCER := This.CCER;
   begin
      return Temp_CCER (Channel).CCxNE = Enable;
   ...
   begin
      ...
      case Polarity is
         when Rising =>
            Temp_CCER (Channel).CCxNP := 0;
            Temp_CCER (Channel).CCxP := 0;
         when Falling =>
            Temp_CCER (Channel).CCxNP := 0;
            Temp_CCER (Channel).CCxP := 1;
         when Both_Edges =>
            Temp_CCER (Channel).CCxNP := 1;
            Temp_CCER (Channel).CCxP := 1;
      end case;
   ...
   procedure Enable_CC_Preload_Control (This : in out Timer) is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Capture_Compare_Preloaded_Control := True;
      This.CR2 := Temp_CR2;
   ...
   procedure Disable_CC_Preload_Control (This : in out Timer) is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Capture_Compare_Preloaded_Control := False;
      This.CR2 := Temp_CR2;
   ...
   procedure Select_Commutation (This : in out Timer) is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Capture_Compare_Control_Update_Selection := True;
      This.CR2 := Temp_CR2;
   ...
   procedure Deselect_Commutation (This : in out Timer) is
      Temp_CR2 : TIMx_CR2 := This.CR2;
   begin
      Temp_CR2.Capture_Compare_Control_Update_Selection := False;
      This.CR2 := Temp_CR2;
   ...
   is
      Temp_OR : TIMx_OR := This.Options;
   begin
      Temp_OR.ITR1_RMP := Option;
      This.Options := Temp_OR;
   ...
   begin
      ...
      case IC1_Polarity is
         when Rising =>
            Temp_CCER (Channel_1).CCxP := 0;
         when Falling =>
            Temp_CCER (Channel_1).CCxP := 1;
         when Both_Edges =>
            Temp_CCER (Channel_1).CCxP := 1;
      end case;
      ...
      case IC2_Polarity is
         when Rising =>
            Temp_CCER (Channel_2).CCxP := 0;
         when Falling =>
            Temp_CCER (Channel_2).CCxP := 1;
         when Both_Edges =>
            Temp_CCER (Channel_2).CCxP := 1;
      end case;
