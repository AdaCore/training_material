package Employee is
   ...
   procedure Set_Name (Item : in out Person_T; Value : Name_T);
   function Name (Item : Person_T) return Name_T;
   procedure Set_Birth_Date (Item : in out Person_T; Value : Date_T);
   function Birth_Date (Item : Person_T) return Date_T;
   procedure Print (Item : Person_T);

   type Employee_T is new Person_T with private;
   procedure Set_Start_Date (Item : in out Employee_T; Value : Date_T);
   function Start_Date (Item : Employee_T) return Date_T;
   procedure Print (Item : Employee_T);

   type Position_T is new Employee_T with private;
   procedure Set_Job (Item : in out Position_T; Value : Job_T);
   function Job (Item : Position_T) return Job_T;
