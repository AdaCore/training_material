...
package body Employee is
   ...
   is (Date.Year'Image & " -" & Date.Month'Image & " -" & Date.Day'Image);

   procedure Set_Name (O : in out Person_T; Value : Name_T) is
   begin
      O.The_Name := Value;
   end Set_Name;
   ...
   procedure Set_Birth_Date (O : in out Person_T; Value : Date_T) is
   begin
   ...
   not overriding
   procedure Set_Start_Date (O : in out Employee_T; Value : Date_T) is
   begin
   ...
   --Types_Body
   not overriding
   procedure Set_Job (O : in out Position_T; Value : Job_T) is
   begin
      O.The_Job := Value;
   end Set_Job;
   not overriding
