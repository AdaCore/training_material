...
package Basics is
   ...
   with
     Pre =>
       (if R.Disc then R.A < Integer'Last else R.B < Integer'Last),
     Post =>
       Value_Rec (R) = Value_Rec (R)'Old + 1;
   ...
   with
     Pre =>
       (if The_Rec.Disc then The_Rec.A < Integer'Last else The_Rec.B < Integer'Last),
     Post =>
       Value_Rec (The_Rec) = Value_Rec (The_Rec)'Old + 1;
