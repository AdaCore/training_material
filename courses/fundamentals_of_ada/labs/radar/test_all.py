...
def adacut(f, answer=True):
    a = [str(CONTRIB / 'adacut.py'),
             "-m", "answer" if answer else "question",
             str(f)]
    r = run(a,
             text=True,
             capture_output=True)
    ...
    if r.returncode:
        ...
        with open(f.parents[1] / ("answers" if answer else "src") / f.name,
                  "wt") as d:
            d.write(r.stdout)
